...
\begin{table}[H]
    ...
    \begin{tabular}{llccc}
        \toprule
        \textbf{黑方 (Black)} & \textbf{白方 (White)} & \textbf{黑方胜率} & \textbf{白方胜率} \\
        \midrule
        MCTS Agent & Random Agent & 90\% & 10\% \\
        Random Agent & MCTS Agent & 0\% & 100\% \\
        \midrule
        Minimax Agent & Random Agent & 100\% & 0\% \\
        Random Agent & Minimax Agent & 0\% & 100\% \\
        \midrule
        MCTS Agent & Minimax Agent & 100\% & 0\% \\
        Minimax Agent & MCTS Agent & 0\% & 100\% \\
        \bottomrule
    \end{tabular}
    \caption{不同AI算法间的10局对战胜率统计}
\end{table}

从上述具体数据可以看出：
\begin{itemize}
    \item MCTS Agent在100次模拟的配置下，能够以90\%的胜率击败Random Agent，显示了其基于统计搜索的强大能力。
    \item Minimax Agent在深度为2的配置下，能够以100\%的胜率击败Random Agent，说明即使是浅层搜索配合合理评估函数也能在小盘上取得压倒性优势。
    \item MCTS Agent在与Minimax Agent的对战中表现出绝对优势，100\%的胜率表明其更适合处理围棋这种高分支因子的复杂游戏。因为Minimax在深度受限的情况下无法充分评估后续局面，而MCTS通过大量模拟能够更好地捕捉到潜在的战术机会和陷阱。
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        \toprule
        \textbf{算法类型} & \textbf{首步平均耗时} & \textbf{计算复杂度} & \textbf{表现稳定性} \\
        \midrule
        Random Agent & $< 0.01\text{s}$ & $O(1)$ & 差 \\
        MCTS ($100$轮) & $0.3\sim0.6\text{s}$ & $O(\text{Rounds} \times \text{Depth})$ & 优 \\
        Minimax (深度2) & $0.02\text{s}$ & $O(B^{\text{Depth}})$ & 中 \\
        \bottomrule
    \end{tabular}
    \caption{算法性能对比结果表}
\end{table}
